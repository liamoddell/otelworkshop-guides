...
### What is Manual SDK Instrumentation?
...
4. **Add OpenTelemetry configuration before `var app = builder.Build();`:**
   ```csharp
...
5. **Edit run.sh to configure service identification:**

   ```bash
   nano /opt/gameserver/run.sh
   ```

6. **Uncomment and set your unique namespace:**
...
3. Look for your service: `gameserver` with namespace `your-name-here`
4. You should see:
   - HTTP requests
...
### Part 1: Add Custom Instrumentation to GameService
...
4. **Wrap PlayGameAsync method with a custom span.** Find the `try` block at the start of PlayGameAsync and add BEFORE it:
   ```csharp
...
5. **Add games started counter.** Find where it logs "Player {PlayerName} is playing" and add AFTER the try { line but BEFORE the logger line:
   ```csharp
   // Increment games started counter
   GamesStartedCounter.Add(1);
   ```

6. **Add span attribute for player name.** Find where it logs "Player {PlayerName} is playing" and add AFTER it:
   ```csharp
   // Add span attribute for player name
   activity?.SetTag("game.player_name", request.Name);
   ```

7. **Add span attributes for game details.** Find where `GetResult` is called and add AFTER it:
   ```csharp
...
8. **Add games completed counter with dimension.** Find where it logs "Game result was {ResultCode}" and add AFTER it:
   ```csharp
...
### Part 2: Add Custom Instrumentation to GetStats Endpoint
...
2. **Custom metrics** in Explore → Metrics:
...
3. **Distributed traces** showing: HTTP request → gameserver → rolldice with custom attributes
