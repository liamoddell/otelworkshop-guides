...
4. **Modify the `java -jar` command to attach the agent:**
...
1. Open Grafana Cloud (URL provided by facilitator)
2. Navigate to **Application Observability**
3. Look for your service: `rolldice` with namespace `your-name-here`
4. You should see:
   - HTTP requests
   - Service map
   - Traces
...
**Why do we need this?** The Java agent automatically instruments the framework, but to add custom spans and metrics, we need direct access to OpenTelemetry's `Tracer` and `Meter` objects. This configuration class makes them available via Spring dependency injection.
...
Lab 4 teaches you to add custom spans, metrics, attributes, events, and error handling. We'll instrument the RollDice controller step by step.
...
4. **Add custom span at the start of rollDice method.** Find the line `String playerName = player.orElse("anonymous");` and add BEFORE it:
   ```java
   ...
   **Right after the `String playerName = player.orElse("anonymous");` line, add:**
   ```java
       // Add span attribute for player name
       span.setAttribute("player.name", playerName);
   ```

5. **Add error handling for validation.** Find the validation error block (playerName.length() > 50) and add span status BEFORE the return statement:
   ```java
...
6. **Add span event for slow responses.** Find the slow response simulation (Thread.sleep) and add AFTER it:
   ```java
...
7. **Add span attribute and custom metric for dice roll.** Find the line `int roll = getRandomNumber(1, 6);` and add AFTER it:
   ```java
...
8. **Add span attributes for streaks.** Find the streak detection block (`if (isStreak)`) and add INSIDE the if block:
   ```java
...
9. **Add error handling for dice malfunction.** Find the dice malfunction error block (currentRequest % 200 == 0) and add BEFORE the return statement:
   ```java
...
10. **Close the span at the end of the method.** At the very end of the rollDice method, BEFORE the final closing brace, add:
    ```java
        } finally {
            span.end();
        }
...
2. In Grafana Cloud, find a trace and you should see:
